...
from restrict_gwas.ldsc import sumstats
from restrict_gwas.ldsc.scripts.ldsc import MASTHEAD, Logger, sec_to_str
...
parser.add_argument("--out", default=None, type=str, help="Output filename prefix.")
...
def munge_sumstats(args, p=True):
    ...
    try:
        ...
        if args.no_alleles and args.merge_alleles:
            raise ValueError("--no-alleles and --merge-alleles are not compatible.")
        if args.daner and args.daner_n:
            raise ValueError(
                "--daner and --daner-n are not compatible. Use --daner for sample "
